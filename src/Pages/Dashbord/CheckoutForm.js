...
const CheckoutForm = ({appointment}) => {
  ...
  useEffect(() =>{
    fetch('https://damp-brook-54993.herokuapp.com/create-payment-intent',{
      method: 'POST',
      headers:{
  ...
  const handleSubmit = async (event) => {
    ...
    const {paymentIntent, error: indentError} = await stripe.confirmCardPayment(
      ...
      if(indentError){
        ...
        fetch(`https://damp-brook-54993.herokuapp.com/booking/${_id}`,{
          method: 'PATCH',
          headers:{
            'content-type': 'application/json',
            'authorization': `Bearer ${localStorage.getItem('accessToken')}`
    ...
    <>
      ...
      }
      {
        success && <div className='text-green-500'>
          <p>{success}</p>
          <p>your transation id: <span className='text-orange-500 font-bold'>{transationId}</span></p>
        </div>
      }
    </>
  );
